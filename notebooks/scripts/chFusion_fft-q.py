...
**Part 2 — Which fusion method is best (per variable)?**
  Four variables x six methods (see ``docs/chfusion_q_energy_peak.md``):
  ...
  - **FFT+q_energy_peak**     ``(q_energy × q_peak)^(1/2)`` on all channels (linear η)
  - **FFT+q_ep_topK**         Top-K by q_energy (linear η, default K=5)
  - **FFT+q_ep_topK_log**     Top-K by q_energy (log η, same K)

**Overview — Full matrix across variables and methods**
  Part 1 only compares variables under Single; Part 2 splits by variable.
  The overview figures put all 24 (variable × method) combinations on one page:

  - Bar chart / heatmap: segment-level mean relative error (leaderboard metrics)
  - Violins by method (6×1): extends Part 1 to all six methods
...
- Console tables: Part 1 variable ranking, Part 2 method tables per variable,
  overall leaderboard (24 combos sorted by mean relative BPM error).
...
- Figures (PDF vector format, English labels; y=0 dashed line = ground truth):
  ...
  - ``chfusion_part2_<variable>_violins.pdf`` — 6 methods x segments per variable
  - **Overview** (all variables × all methods):
    - ``chfusion_overview_4x3_mean_error_bars.pdf`` — grouped bar chart (mean ± std)
    - ``chfusion_overview_4x3_heatmap.pdf`` — mean error heatmap
    - ``chfusion_overview_4x3_violins_by_method.pdf`` — 6×1 panels: per method, 4 variables
    - ``chfusion_overview_4x3_violins_by_variable.pdf`` — 4×1 panels: per variable, 6 methods
...
# energy_peak_top_k: Top-K prefilter for FFT+q_ep_topK (see docs/chfusion_q_energy_peak.md).
...
# | ``overview_4x3_mean_error_bars`` | 24 combos: mean segment rel err ± std |
# | ``overview_4x3_heatmap`` | Same 24 combos as colour matrix |
# | ``overview_4x3_violins_by_method`` | 6×1 stack: 4 variables per method |
# | ``overview_4x3_violins_by_variable`` | 4×1 stack: 6 methods per variable |
#
