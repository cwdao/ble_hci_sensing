...
时，进入 Advisor Briefing Mode。

------

## 10. Advisor Briefing Mode 职责

### 10.1 定位

导师汇报面向**没有亲自参与实验的研究者**（导师、合作者、审稿人）。它与成果汇报（Achievement Report Mode, §10）的本质区别在于：

| 维度 | 成果汇报 (Achievement Report) | 导师汇报 (Advisor Briefing) |
|------|------------------------------|---------------------------|
| 读者 | 人（阅读与决策） | **未参与实验的导师/合作者** |
| 背景需求 | 对项目有基本了解 | **需要从零建立物理/数据直觉** |
| 风格 | 论证性、逐假设讨论 | **叙事性、教学性、先建背景再讲发现** |
| 图表 | 内嵌 `![]()` | 表格为主（导师可能不打开图文件） |
| 核心目标 | 呈现结论 | **呈现困惑 + 征求建议** |
| 结尾 | 部署建议 | **向导师提出的具体问题列表** |
...
```text
docs/achievements/{topic}_advisor_briefing.md
```
...
导师汇报**必须**从零开始建立物理直觉。这一节的读者假设是"知道 BLE 是什么但没做过 CS 感知"。

必须包含：
- BLE CS 的测量机制（≤3 个要点，不要公式堆砌）
- 三种可用变量的物理含义 + 为什么 Total amplitude 不可用（一句话表格）
- 呼吸感知的核心挑战（为什么从 216 维信道信息中提取呼吸信号不是平凡问题）
- 实验数据概况（数据集表：HKH vs CS，规模，GT 类型）

**禁止**：用"详见 plan §2"跳过——导师不会去读 plan。

#### §2. 实验历程（简短）

- 不超过一页
- 用时间线/箭头图展示方法的演化逻辑（不要平铺所有实验）
- 重点标注**在哪里出现了预期之外的发现**（这是导师最有兴趣的）

#### §3. 核心发现与困惑

- 当前状态：什么有效、什么无效
- 关键数字（用表，不用图）
- **突出的困惑或矛盾**（这才是导师能提供价值的地方）
...
- **这是导师汇报的核心价值所在**
- 每个问题必须具体、可操作（不能是"您觉得怎么样"这种开放题）
- 覆盖：物理层面、方法论层面、实验设计层面、叙事/写作层面、优先级层面
- 建议 4-6 个问题
...
- **第一页必须让导师理解物理原理**：如果导师读到 §2 还不清楚"为什么 72 tone 比 1 tone 好"，汇报是失败的
- **表格优先于图表**：导师可能不在 IDE 中打开 `.png`，表格可以在任何地方阅读
- **所有数字必须来自实际实验结果**（`outputs/reports/`），不得编造或估算
- **诚实报告失败实验**——这通常是导师最有助于诊断的部分
- **不确定处标 `[待确认]`**
- 方法名使用描述性名称（如"逐模态 Voting → 三模态等权谱融合"），不得仅用代号（B1/T0-V3/G4）
- **结尾必须有向导师提出的具体问题**，否则导师不知道如何反馈

### 10.5 反模式（不要做的事）

- ❌ 复制 plan 的 §2（物理与变量）全文——太长、太公式化
- ❌ 用项目内部代号（B1/B3/G4）而不给描述性名称
- ❌ 把 CS 和 HKH 的结果混在一张表里而不分域标注
- ❌ 把单场景结论写成全局有效
- ❌ 只放图表路径而不给数字表（导师不会逐个打开 `outputs/figures/`）
- ❌ 结尾没有向导师提出的具体问题（"请您提意见"不是具体问题）
- ❌ 假设导师记得上次汇报的细节（新汇报应自包含）
...
## 13. Claude/DeepSeek 给 Cursor Composer 的交接
...
## 14. Cursor Composer 执行后返回给 Claude/DeepSeek 的材料
