...
| **FFT+q_ep_topK** | Top-K by q_energy (**linear** η map), then energy+peak fusion |
| **FFT+q_ep_topK_log** | Same as topK, but q_energy uses **log** map (same formula as q_peak) |
...
- **映射到 [0, 1]** — 两种模式（`_quality_from_energy_ratio(..., log_map=)`）：

  **线性**（`FFT+q_energy_peak`、`FFT+q_ep_topK`）：

  $$q_{\text{energy},c} = \operatorname{clip}\left(\frac{\eta_c - \eta_{\min}}{\eta_{\mathrm{good}} - \eta_{\min}},\, 0,\, 1\right)$$

  **对数**（`FFT+q_ep_topK_log`，与 q_peak 相同形式）：

  $$q_{\text{energy},c} = \operatorname{clip}\left( \frac{\log \eta_c - \log \eta_{\min}} {\log \eta_{\mathrm{good}} - \log \eta_{\min}},\, 0,\, 1 \right)$$

  默认阈值：`energy_ratio_min = 0.02`，`energy_ratio_good = 0.20`（两种映射共用）。
...
### 5.1 Top-K + 对数 q_energy（FFT+q_ep_topK_log）

与 §5 流程相同，但 **Top-K 排序** 与 **√(q_energy·q_peak) 中的 q_energy** 均改用 §2.1 的对数映射（`_quality_from_snr` 同款公式，阈值仍用 `energy_ratio_min/good`）。

**动机**：线性 η 映射在 η 接近 `η_good` 时区分度下降；对数映射与 q_peak 一致，对低 η 信道惩罚更重、高 η 之间拉开差距。

**方法键**：`fft_q_energy_peak_topk_log_fusion`（benchmark 标签 `FFT+q_ep_topK_log`）
...
| `energy_ratio_min` / `energy_ratio_good` | 0.02 / 0.20 | q_energy 线性或对数映射 |
...
Part 2 默认对比（4 变量 × 6 方法）：

1. Single  
2. Uniform  
3. FFT+q_peak  
4. FFT+q_energy_peak（全信道，线性 q_energy）  
5. FFT+q_ep_topK（Top-K，线性 q_energy）  
6. FFT+q_ep_topK_log（Top-K，对数 q_energy）
...
| 2026-06-01 | 默认 Top-K 从 20 调整为 5（metal-plate 数据验证） |
| 2026-06-01 | 新增 `FFT+q_ep_topK_log`：Top-K + energy+peak，q_energy 改用对数映射 |
