...
| 验证域 | 状态 | 说明 |
|--------|------|------|
| **HKH 12 场景** | ✅ 已验证 | BPM **0.405**（≡ B1），RMSE **0.950**（≡ B2-D）；来源 `ble_hkh_b3_simplified_validation_summary.json` |
| **CS 金属板三场景**（091339 / 095806 / 102621） | **[待确认]** | B3 Simplified **尚未**在 CS 场景跑批量验证；BPM 是否 ≡ B1（跨域 8.45%）**未证实** |

> CS 场景标注 **[待确认]** 的含义：当前推荐状态仅基于 HKH 真人数据；CS 金属板脚本场景需单独验证后方能推广结论。
...
| 12 | B 系列·波形 | B2-A0 PCA 符号 MRC | 12.33% | 20.57 | 6.74 | 9.69 | ✅ | 实验（不推荐部署） |
| 13 | **Z1-no-VMD** | Zhuo2023 PCA→PCA→Peak | **11.21%** | 18.70 | 7.71 | 7.23 | ✅ | 外部 baseline（已结案） |
| 14 | **Z1** | Zhuo2023 PCA→PCA→VMD→Peak | **11.31%** | 17.49 | 8.20 | 8.22 | ✅ | 外部 baseline（已结案） |
| 15 | MRC-PCA-η-equal-pca | √η-MRC → 三波形 PCA(3→1) | 12.79% | 24.43 | 6.84 | 7.10 | ✅ | 外部（修正变体，已结案） |
| 16 | Fan-η-equal-wf | η-MRC → 三波形时域等权 | 16.60% | 23.09 | 11.00 | 15.72 | ✅ | 外部（修正变体，已结案） |

> **2026-06-26 更新**：追加 Z1 系列（§4.10）与 WiFi MRC equal 修正变体（§4.9）。跨域排序上：**B1 < B2-D ≈ Modal top2 < B0 < WiFi MRC legacy < Z1 < MRC 波形修正变体**。
