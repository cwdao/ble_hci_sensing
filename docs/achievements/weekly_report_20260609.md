...
```text
Voting 信道策略
    ├── 效应 A：抑制信道间相关噪声 → per-modal BPM 更准（Deng et al.）
    └── 效应 B：降低模态间频谱差异性 → Equal > Top2（本发现）
                                          ↓
                              B1 = 8.45%（效应 A + B 叠加）
```

**证明边界**（重要）：图 4 证明效应 B（模态间关系变化），不直接证明效应 A（Voting > Single-best）。效应 A 的证据来自 BPM 数字：T0-V3（9.20%）< Single Remote（10.45%）。
